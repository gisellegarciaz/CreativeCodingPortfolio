...
import IntroVideo from "../../assets/video/IntroMotion.webm";
...
export default function StickersIntro({ onComplete }) {
    ...
    const lines = ["POWERFUL", "DESIGN", "CREATIVE", "CODING"];

    useEffect(() => {
        const tl = gsap.timeline();

        // 1. ENTRADA DAS LETRAS (Mantemos para preencher o fundo)
        tl.fromTo(".giant-char",
            { opacity: 0, y: 100, rotateX: -90 },
            { opacity: 1, y: 0, rotateX: 0, duration: 1, stagger: 0.03, ease: "power4.out" }
        );

        // 2. EFEITO DE PISCAR AS PALAVRAS (Ping-pong enquanto o vídeo rola)
        const blinkTl = gsap.timeline({ repeat: -1 });
        lines.forEach((_, i) => {
            blinkTl.to(`.line-${i} .giant-char`, { color: "rgba(255, 255, 255, 0.15)", duration: 0.4 })
                .to(`.line-${i} .giant-char`, { color: "transparent", duration: 0.4 }, "+=0.2");
        });

        // 3. SAÍDA DA TELA (Gatilhada quando o vídeo termina)
        if (videoEnded) {
            const exitTl = gsap.timeline({
                onComplete: () => { if (onComplete) onComplete(); }
            });

            exitTl.to([videoRef.current, ".marquee-container", ".liquid-bg"], {
                y: 1200,
                duration: 0.6,
                ease: "power4.in"
            })
                .to(containerRef.current, {
                    ...
                    ease: "power4.in",
                    onStart: () => {
                        // Mantém o GiantText "parado" na tela enquanto o resto desce
                        gsap.to(giantTextRef.current, { yPercent: -100, duration: 0.8, ease: "power4.in" });
                    }
                }, "-=0.4");
        }
                ...
                {lines.map((word, i) => (
                    <GiantLine key={i} className={`line-${i}`}>
                        {word.split("").map((char, j) => (
                            <Char key={j} className="giant-char">{char}</Char>
                        ))}
                    </GiantLine>
                ))}
            </GiantTextContainer>
